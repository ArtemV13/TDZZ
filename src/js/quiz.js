...
(function () {
  ...
  /* бейдж: сколько заявок сегодня (api/stats.php) */
  fetch(root + 'api/stats.php').then(function (r) { return r.json(); }).then(function (j) {
    if (j && j.today > 0) $('qzBadge').textContent = t('Сегодня уже ' + j.today + ' ' + plural(j.today, ['заявка', 'заявки', 'заявок']) + ' · отвечаем в течение 15 минут', j.today + ' enquiries today · we reply within 15 minutes');
  }).catch(function () {});
    ...
    cards.querySelectorAll('.qc').forEach(function (c) {
      c.addEventListener('click', function () {
        ...
        if (q.multi) {
          ...
          if (q.key === 'quality') { if (v !== 'approx') ['protein', 'gluten', 'moisture', 'trash', 'natura', 'oil'].forEach(function (k) { delete extra[k]; }); if (v === 'none') { files = []; delete extra.comment; } renderExtra(v); } else setTimeout(next, 220);
        }
        side();
      });
    });
    renderExtra(q.key === 'quality' ? ans.quality : null);
    $('qzBack').style.visibility = i ? 'visible' : 'hidden';
    side();
  }
